...
final class AuthorsGetController extends Controller
{
    ...
    {
        return isset($request['course_id'])
            ? $this->authorsByCourse($request['course_id'])
            : $this->jsonResponse(self::ALL_AUTHORS);
    }

    private function authorsByCourse(string $courseId): string
    {
        if (!isset(CoursesGetController::ALL_COURSES[$courseId])) {
            $this->notFoundResponse("The course `$courseId` doesn't exist");
        }

        return $this->jsonResponse(CoursesGetController::ALL_COURSES[$courseId]['authors']);
    }
}
